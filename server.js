...
function roomState(r){return {type:'roomState',room:r.code,mode:r.mode,host:r.host,started:r.started,teamScore:r.coopScore||0,teamLines:r.coopLines||0,teamCombo:r.coopCombo||0,players:[...r.players.values()].map(p=>({id:p.id,name:p.name,ready:p.ready,score:p.score,alive:p.alive,board:p.board||null,active:p.active||null}))}}
...
function makeRoom(c,mode,host){return {code:c,mode:mode==='coop'?'coop':'versus',host,started:false,seed:crypto.randomInt(1,0x7fffffff),players:new Map(),createdAt:Date.now(),lastActivity:Date.now(),coopBoard:emptyBoard(),coopScore:0,coopLines:0,coopCombo:0,coopLevel:1}}
function leave(p){if(!p.room)return;const r=rooms.get(p.room);p.room=null;if(!r)return;r.players.delete(p.id);if(r.host===p.id)r.host=r.players.values().next().value?.id||null;if(r.started)r.started=false;if(!r.players.size)rooms.delete(r.code);else broadcastState(r);broadcastLobbies()}
function resetPlayer(p){p.ready=false;p.score=0;p.alive=true;p.board=null;p.active=null}
function startIfReady(r){if(r.started||!r.players.size||![...r.players.values()].every(p=>p.ready))return;r.started=true;r.seed=crypto.randomInt(1,0x7fffffff);r.coopBoard=emptyBoard();r.coopScore=0;r.coopLines=0;r.coopCombo=0;r.coopLevel=1;touch(r);for(const p of r.players.values()){p.score=0;p.alive=true;p.board=null;p.active=null}broadcast(r,{type:'start',seed:r.seed,mode:r.mode});broadcastState(r)}
...
function coopLock(r,m){if(!r.started||r.mode!=='coop'||!Array.isArray(m.matrix))return;const x=Number(m.x),y=Number(m.y),c=Math.max(0,Math.min(6,Number(m.c)||0))+1;for(let yy=0;yy<m.matrix.length;yy++)for(let xx=0;xx<(m.matrix[yy]?.length||0);xx++)if(m.matrix[yy][xx]){const bx=x+xx,by=y+yy;if(bx>=0&&bx<10&&by>=0&&by<20)r.coopBoard[by][bx]=c}const rows=clearBoard(r.coopBoard);const n=rows.length;if(n){const base=[0,100,300,500,800][n]||1200;r.coopCombo++;r.coopLines+=n;r.coopLevel=1+Math.floor(r.coopLines/10);r.coopScore+=base*r.coopLevel+Math.min(r.coopCombo,12)*50}else r.coopCombo=0;touch(r);broadcast(r,{type:'coopState',board:r.coopBoard,score:r.coopScore,lines:r.coopLines,combo:r.coopCombo,level:r.coopLevel,rows,event:n?`clear:${n}`:null});broadcastState(r)}
